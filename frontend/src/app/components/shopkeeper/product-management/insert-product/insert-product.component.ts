...
export class InsertProductComponent implements OnInit {
  ...
  selectFiles(event) {
    ...
    // localStorage.setItem('fileArray', JSON.stringify(event.target.files));
    ...
    this.rawfileName = '';
    // console.log(JSON.parse(localStorage.getItem('fileArray')));
    // this.uploadFiles();

  }

  uploadFiles() {
    // this.selectedFiles = JSON.parse(localStorage.getItem('fileArray'));
  ...
  insertProduct(pform: NgForm, productData: ProductModel) {
      if (this.isSelectMcat) {
        if (this.sCatid) {
          ...
          productData.slug = String(this.uniqueNumber());
